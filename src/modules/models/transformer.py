...
class SelfAttention(nn.Module):
    ...
    def attention(self, query, key, value, mask=None):
        B, C, T = query.size()
        query = query.view(B, self.n_heads, self.inter_channels, T).transpose(2, 3)
        ...
        output = output.transpose(2, 3).contiguous().view(B, C, T)
        return output
    ...
    def _relative_position_to_absolute_position(self, x):
        ...
        x_flat = F.pad(x_flat, convert_pad_shape([[0, 0], [0, 0], [0, L - 1]]))
